...
class EnergyGame {
    ...
    async initializeGlobalState() {
        try {
            if (typeof GlobalGameState !== 'undefined') {
                this.globalGameState = new GlobalGameState();
                await this.globalGameState.loadGameData();
                
                // Sync energy keys from global state
                this.syncEnergyKeysFromGlobal();
                
                console.log('Global game state initialized successfully');
            } else {
                console.log('GlobalGameState not available, using local storage');
                this.loadLocalEnergyKeys();
            }
        } catch (error) {
    ...
    syncEnergyKeysFromGlobal() {
        if (!this.globalGameState) return;
        
        const keyMapping = {
            'living': 'ruangTamu',
            'kitchen': 'dapur', 
            'lab': 'laboratorium',
            'basement': 'ruangBawahTanah'
        };
        
        Object.keys(keyMapping).forEach(localKey => {
            const globalKey = keyMapping[localKey];
            this.energyKeys[localKey] = this.globalGameState.hasEnergyKey(globalKey);
        });
    ...
    loadLocalEnergyKeys() {
        try {
            const savedKeys = JSON.parse(localStorage.getItem('energyGameKeys') || '{}');
            this.energyKeys = { ...this.energyKeys, ...savedKeys };
        } catch (error) {
            console.error('Error loading local energy keys:', error);
        }
    }
    ...
    completeZone(zoneId) {
        ...
        this.saveEnergyKeyToGlobal(zoneId);
    ...
    saveEnergyKeyToGlobal(zoneId) {
        ...
        const globalKey = keyMapping[zoneId];
        
        if (this.globalGameState && globalKey) {
            this.globalGameState.collectEnergyKey(globalKey);
            this.globalGameState.saveGameData();
            console.log(`Energy key ${globalKey} saved to global state`);
        } else {
            // Fallback to local storage
            const savedKeys = JSON.parse(localStorage.getItem('energyGameKeys') || '{}');
            savedKeys[zoneId] = true;
            localStorage.setItem('energyGameKeys', JSON.stringify(savedKeys));
            console.log(`Energy key ${zoneId} saved to local storage`);
        }
    }
